...
describe("escrow", () => {
  ...
  test("Offer created by Alice and vault are closed, tokens and rent are returned", async () => {
    const offeredUsdc = new BN(10_000_000);
    const wantedWif = new BN(100_000_000);
    ...
    // SOL rent for Offer and Vault is refunded
    const aliceSolAfter = await connection.getBalance(alice.publicKey);
    expect(aliceSolAfter).toBeGreaterThan(aliceSolBefore);
  });
});
